...
* Set Bandings
*/
function my_setBandings( theme, header, footer, doc ) {
  theme = theme || 'LIGHT';
  doc   = doc   || my_getDocObj();

  var bandingTheme = BandingTheme[ theme ];
  var showHeader   = header || false;
  var showFooter   = footer || false;

  my_removeBandings( doc.range );
  my_applyRowBanding( doc.range, bandingTheme, showHeader, showFooter );
}
